...
class RatingsController extends RatingAppController {
   ...
   */
 	...
 	var $uses = array('Transaction', 'Marker');
  ...
  function view($model = '', $id = 0, $options = '') {
    ...
    $this->set('data', array('%VOTES%' => $votes.' '.__n('Votes', 'Vote', $votes, true), 
                             '%RATING%' => $userRating, 
                             '%AVG%' => $avgRating,
  ...
  function save($model = '', $id = 0, $value = 0) {
    ...
    if (!empty($userRating) && Configure::read('Rating.allowChange')) {
      ...
      if ($userRating['Rating']['rating'] == $value && Configure::read('Rating.allowDelete')) {
		...
		$this->$model->Transaction->saveField('marker_id', $id);
		$this->$model->Transaction->saveField('name', __('Marker has been rated',true));
   ...
   */
  function __loadConfig($config) {
    if (Configure::load($config) === false) {
