...
class UsersController extends AppController {
	...
	var $components = array('RequestHandler', 'Email', 'MathCaptcha', 'Cookie', 'Ticketmaster', 'Notification');
		...
		if (!empty($this->data)) {
			if ($this->MathCaptcha->validates($this->data['User']['security_code'])) {
				...
				if ($this->User->save($this->data)) {
					...
					$this->Notification->sendMessage("welcome", null, $recipient);
	...
	function confirm($id) { 
			...
			if ($this->User->saveField('active', 1)) {
				...
				$recipient = $this->data['User']['email_address'];		
				$this->Notification->sendMessage("userdata", $this->Marker->id, $recipient);
	...
	function resetpassword($email=null) {
		...
		if (!empty($this->data['User']['email'])) {
			if ($this->MathCaptcha->validates($this->data['User']['security_code'])) {
				...
				$recipient = $email;		
				$this->Notification->sendMessage("resetpw", null, $recipient);
